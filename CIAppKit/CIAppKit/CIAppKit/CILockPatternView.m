...
//  CILockPatternView
//
//  Updated by CI-Dapan.Lee on 20-12-4.
//  Repeatable selection for LockPatternView.
//
//  Created by ZhangTinghui on 13-7-17.
//  Copyright (c) 2013年 ZhangTinghui. All rights reserved.
//  SourcePath: https://github.com/Tinghui/HUIPatternLockView

#if !__has_feature(objc_arc)
#error This file must be compiled with ARC. Use -fobjc-arc flag (or convert project to ARC).
#endif

#import "CILockPatternView.h"

#pragma mark - @class CILockPatternViewDot
@interface CILockPatternViewDot : NSObject
...
@implementation CILockPatternViewDot

+ (instancetype)dotWithNumber:(NSUInteger)number frame:(CGRect)frame {
    CILockPatternViewDot *dot = [[self alloc] init];
    dot.number  = number;
    dot.frame   = frame;
...
#pragma mark - @class CILockPatternView
...
@interface CILockPatternView () {
    //Update by CI-Dapan.Lee
    CILockPatternViewDot *currentDot;
}
...
@implementation CILockPatternView
    ...
    for (NSInteger i = 0; i < self.numberOfRows; i++) {
        for (NSInteger j = 0; j < self.numberOfColumns; j++) {
            ...
            CGRect dotFrame = CGRectMake(dotCenter.x - self.dotWidth * 0.5,
                                         ...
                                         self.dotWidth,
                                         self.dotWidth);
            [self.normalDots addObject:[CILockPatternViewDot dotWithNumber:(number++) frame:dotFrame]];
        }
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.normalDotImage != nil) {
        for (CILockPatternViewDot *dot in self.normalDots) {
            [self.normalDotImage drawInRect:dot.frame];
        }
    }
    
    if (self.highlightedDotImage != nil) {
        for (CILockPatternViewDot *dot in self.highlightedDots) {
...
- (CILockPatternViewDot *)_normalDotContainsPoint:(CGPoint)point {
    for (CILockPatternViewDot * dot in self.normalDots){
...
- (void)_updateLinePathWithPoint:(CGPoint)point {
    ...
    CILockPatternViewDot *dot = [self _normalDotContainsPoint:point];
